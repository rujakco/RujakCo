...
function buildLogPayload({ level, context, message, error = null, meta = {} }) {
  ...
  const safeMeta = { ...meta };
  delete safeMeta.context; delete safeMeta.level; delete safeMeta.message; delete safeMeta.stack;
  const payload = {
    context,
    level,
...
function sendToSupabase(payload) {
  try {
    ...
    if (!sb) return;
    sb.from('error_logs').insert(payload)
      .then(() => { if (isDevelopment()) console.debug('[Logger] Log sent to Supabase:', payload.context); })
      .catch((err) => { if (isDevelopment()) console.warn('[Logger] Failed to send log to Supabase:', err); });
...
export function withErrorLogging(fn, context, meta = {}) {
  ...
  if (isAsync) {
    return async (...args) => {
      try { return await fn(...args); } catch (error) { await logError(context, error, { ...meta, args }); throw error; }
    };
  }
  return (...args) => {
    try { return fn(...args); } catch (error) { logError(context, error, { ...meta, args }).catch(() => {}); throw error; }
  };
}
