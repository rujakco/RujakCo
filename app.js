    ...
    const updateCenter = () => {
      ...
      if (closestItem) {
        ...
        if (state.currentInsightId !== newId) {
          state.currentInsightId = newId;
          const prod = PRODUCTS.find(p => p.id === newId);
          const insightEl = document.getElementById('productInsightText');
          if (insightEl && prod) {
