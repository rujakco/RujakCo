// app.js — VERSI FINAL LENGKAP + SEMUA PATCH AUDIT
...
function closeModal(modalEl, fromPopState = false) {
  ...
  releaseInert();
  if (!fromPopState) { isProgrammaticBack = true; history.back(); }
  syncBottomNav();
}
...
// ---------------------------------------------------------------------------
function openProductPage(globalIndex) { /* ... tidak berubah ... */ }
function closeProductPage(fromPopState = false) { /* ... tidak berubah ... */ }

// ---------------------------------------------------------------------------
// GESTURES (tidak berubah)
// ---------------------------------------------------------------------------
function initDetailGestures() { /* ... tidak berubah ... */ }

// ---------------------------------------------------------------------------
// CART & SHIPPING (tidak berubah)
// ---------------------------------------------------------------------------
function getCartSummaryLocal() { return getCartSummary(state.cart); }
function updateShippingUI() { /* ... tidak berubah ... */ }
function updateCartUI() { /* ... tidak berubah ... */ }
...
function initDrawerDistrictDropdown() {
  ...
  input.addEventListener('keydown', (e) => {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); if (activeOptionIndex >= 0 && activeOptionIndex < opts.length) { const opt = opts[activeOptionIndex]; selectDrawerDistrict(parseFloat(opt.dataset.lat), parseFloat(opt.dataset.lon), opt.dataset.name); } }
    else if (e.key === 'Escape') { dropdown.style.display = 'none'; input.setAttribute('aria-expanded', 'false'); input.focus(); }
  });
  dropdown.addEventListener('click', (e) => { const opt = e.target.closest('[role="option"]'); if (opt) selectDrawerDistrict(parseFloat(opt.dataset.lat), parseFloat(opt.dataset.lon), opt.dataset.name); });
...
function initOnboarding() {
  ...
  if (saved?.name && saved.district) {
    ...
    // ✅ Jika returning user, langsung tampilkan konten utama
    DOM.onboardingOverlay.classList.add('hidden');
    setTimeout(() => { DOM.onboardingOverlay.style.display = 'none'; DOM.mainContent?.removeAttribute('inert'); if(DOM.bottomNav) DOM.bottomNav.style.display = 'flex'; }, 600);
  ...
  const selectOnbDistrict = async (lat, lon, name) => {
    dropdown.style.display = 'none'; input.setAttribute('aria-expanded', 'false'); input.value = 'Menghitung…';
  ...
  input.addEventListener('input', debounce(async (q) => { if(q.length<3){dropdown.style.display='none'; input.setAttribute('aria-expanded','false'); return} dropdown.innerHTML='<div style="padding:14px;text-align:center;color:var(--gray-500)">Mencari…</div>'; dropdown.style.display='block'; input.setAttribute('aria-expanded','true'); let r=[]; try{r=await queuedSearch(q)}catch{return} renderOnbDropdown(r); },700));
  input.addEventListener('keydown', (e) => { const opts = dropdown.querySelectorAll('[role="option"]'); if(!opts.length||dropdown.style.display==='none')return; if(e.key==='ArrowDown'){e.preventDefault();setActive(Math.min(activeOptionIndex+1,opts.length-1))} else if(e.key==='ArrowUp'){e.preventDefault();setActive(Math.max(activeOptionIndex-1,0))} else if(e.key==='Enter'&&activeOptionIndex>=0){e.preventDefault();const o=opts[activeOptionIndex];selectOnbDistrict(+o.dataset.lat,+o.dataset.lon,o.dataset.name)} else if(e.key==='Escape'){dropdown.style.display='none';input.setAttribute('aria-expanded','false');input.focus();} });
  ...
  document.getElementById('onbStartBtn').addEventListener('click', () => { if(!state.selectedDistrict) return showToast('Mohon pilih alamat tujuan.'); saveUser(state.customerName, state.selectedDistrict); DOM.onboardingOverlay.classList.add('hidden'); setTimeout(() => { DOM.onboardingOverlay.style.display = 'none'; if(DOM.bottomNav) DOM.bottomNav.style.display = 'flex'; DOM.mainContent?.removeAttribute('inert'); }, 600); applyPersonalization(); initScrollReveal(); });
  document.getElementById('onbEnterBtn').addEventListener('click', () => { DOM.onboardingOverlay.classList.add('hidden'); setTimeout(() => { DOM.onboardingOverlay.style.display = 'none'; if(DOM.bottomNav) DOM.bottomNav.style.display = 'flex'; DOM.mainContent?.removeAttribute('inert'); }, 600); applyPersonalization(); initScrollReveal(); });
  document.getElementById('onbResetBtn').addEventListener('click', () => { clearUser(); state.cart={}; updateCartUI(); DOM.onbReturningUser.style.display='none'; DOM.onbNewUser.style.display='block'; DOM.onbStep2.classList.remove('active'); DOM.onbStep1.classList.add('active'); if(DOM.bottomNav) DOM.bottomNav.style.display='none'; DOM.mainContent?.setAttribute('inert', ''); });
}

// ---------------------------------------------------------------------------
// WHATSAPP, TELEGRAM & DOWNLOAD STRUK (lengkap)
// ---------------------------------------------------------------------------
async function downloadReceiptPNG() {
  ...
  try {
    ...
    const blob = await new Promise(r => canvas.toBlob(r, 'image/png')); if (!blob) return null;
...
async function sendReceiptToTelegram() {
  ...
  const supabase = getSupabase(); if (!supabase) { console.error('Supabase client tidak tersedia'); return; }
...
async function sendReceiptToWhatsApp() {
  const summary = getCartSummaryLocal(); if (summary.items.length === 0) { showToast('❌ Keranjang kosong.'); return; }
  ...
  if (state.shippingProvider === 'rujakco') { logisticInfo += ` (${state.vehicleType === 'mobil' ? 'Mobil' : 'Motor'})`; if (state.isPriority) logisticInfo += ' [PRIORITAS]'; }
  const shipCost = DOM.finalShipping?.textContent || '—'; let totalCost = DOM.finalTotal?.textContent || '—';
  ...
  msg += `\n💵 *Subtotal:* ${fmt(summary.subtotal)}\n🛵 *Ongkir:* ${shipCost}\n💰 *TOTAL TRANSFER:* *${totalCost}*\n\n📎 _Mohon lampirkan *gambar bukti transfer (QRIS)* Anda di sini agar reservasi dapat segera kami proses._`;

  const sb = getSupabase(); if (sb) { try { await sb.from('orders').insert({ order_code: state.currentOrderCode, customer_name: name, customer_phone: phone, customer_address: address, district: state.selectedDistrict, distance_km: state.userDistance, items: summary.items, subtotal: summary.subtotal, shipping_cost: parseInt(shipCost.replace(/\D/g, '')) || null, total: parseInt(totalCost.replace(/\D/g, '')) || null, shipping_provider: logisticInfo, delivery_time: deliveryTime, notes, status: 'pending_payment' }); } catch (err) { console.error("Gagal menyimpan ke database:", err); } }
...
function bindEvents() {
  ...
  document.getElementById('shareProductBtn')?.addEventListener('click', () => {
    ...
    const productId = PRODUCTS[currentIndex % PRODUCTS.length]?.id; if (!productId) return;
  ...
  document.getElementById('navHomeBtn')?.addEventListener('click', () => {
    ...
    window.scrollTo({ top: 0, behavior: 'smooth' }); setActiveNav('navHomeBtn');
  ...
  document.getElementById('aiChatToggle')?.addEventListener('click', (e) => {
    ...
    if (DOM.paymentModal?.classList.contains('active')) closeModal(DOM.paymentModal);
    if (document.getElementById('orderConfirmModal')?.classList.contains('active')) closeModal(document.getElementById('orderConfirmModal'));
    openModal(DOM.aiChatBox); setTimeout(() => { isNavClick = false; }, 100);
  });

  // delivery time dropdown (tidak berubah)
  const deliveryTrigger = document.getElementById('deliveryTimeTrigger'), deliveryDropdown = document.getElementById('deliveryTimeDropdown'), deliveryHidden = document.getElementById('deliveryTime'), deliveryLabel = document.getElementById('deliveryTimeLabel');
  ...
  function setDeliveryOption(option) { deliveryDropdown.querySelectorAll('[role="option"]').forEach(o => o.setAttribute('aria-selected', 'false')); option.setAttribute('aria-selected', 'true'); deliveryLabel.textContent = option.textContent; deliveryHidden.value = option.dataset.value; closeDeliveryDropdown(); }
  ...
  document.addEventListener('click', async (e) => {
    ...
    const boutique = e.target.closest('.boutique-item'); if (boutique) { const idx = parseInt(boutique.dataset.idx); if (!isNaN(idx)) openProductPage(idx); return; }
    ...
    const spiceOption = e.target.closest('.spice-option'); if (spiceOption) { const pid = spiceOption.dataset.pid, val = parseInt(spiceOption.dataset.spice); state.drafts[pid].spice = val; document.querySelectorAll(`.spice-option[data-pid="${pid}"]`).forEach(b => b.classList.toggle('active', parseInt(b.dataset.spice) === val)); document.querySelectorAll(`[id^="spiceLabel_"][id$="_${pid}"]`).forEach(el => el.textContent = SPICE_LABELS[val]); return; }
    const qtyPlus = e.target.closest('.qty-plus'), qtyMinus = e.target.closest('.qty-minus'); if (qtyPlus || qtyMinus) { const pid = (qtyPlus || qtyMinus).dataset.pid; if (qtyPlus) state.drafts[pid].qty++; else if (state.drafts[pid].qty > 1) state.drafts[pid].qty--; document.querySelectorAll(`.qty-num[data-valpid="${pid}"]`).forEach(el => el.textContent = state.drafts[pid].qty); return; }
    const addBtn = e.target.closest('.add-to-cart-btn');
    if (addBtn) {
      if (window.navigator.vibrate) window.navigator.vibrate(10);
      const pid = addBtn.dataset.pid, idx = addBtn.dataset.idx, draft = state.drafts[pid], cartKey = pid + '_spice' + draft.spice;
      ...
      setTimeout(() => { const step1 = document.getElementById(`step1_${idx}_${pid}`), step2 = document.getElementById(`step2_${idx}_${pid}`); if (step1 && step2) { step1.style.display = 'block'; step2.style.display = 'none'; step1.style.opacity = '1'; } }, 500);
      return;
    }
    ...
    const actionBtn = e.target.closest('[data-action]'); if (actionBtn && !actionBtn.classList.contains('add-to-cart-btn') && !actionBtn.classList.contains('step-1-btn')) { const id = actionBtn.dataset.id, type = actionBtn.dataset.action; if (type === 'increase') state.cart[id].qty++; else if (type === 'decrease') { if (state.cart[id].qty === 1) { showConfirmModal('Hapus Sajian?', 'Sajian ini akan dihapus dari reservasi Anda.', () => { delete state.cart[id]; updateCartUI(); if (DOM.miniCartModal.classList.contains('active')) renderMiniCart(state.cart); showToast('Sajian dihapus dari reservasi.'); }); return; } state.cart[id].qty--; } updateCartUI(); if (DOM.miniCartModal.classList.contains('active')) renderMiniCart(state.cart); return; }
    ...
    if (e.target.id === 'btnOpenPayment') {
      ...
      const phone = DOM.customerPhoneInput?.value.trim() || '', address = DOM.customerAddressInput?.value.trim() || '';
      ...
      if (state.userDistance == null) {
        ...
        if (addressToSearch) { try { const results = await queuedSearch(addressToSearch); if (results.length > 0) { const place = results[0]; const result = await getDrivingDistance(SYSTEM.STORE_LAT, SYSTEM.STORE_LNG, parseFloat(place.lat), parseFloat(place.lon)); state.userDistance = result.distance; state.selectedDistrictFull = place.display_name; state.selectedDistrict = extractShortLocation(place.display_name); DOM.districtInput && (DOM.districtInput.value = place.display_name); saveCustomer(phone, address, place.display_name, result.distance); updateShippingUI(); recovered = true; } } catch (err) { console.warn('Auto-recover gagal:', err); } }
      ...
      const receiptOk = await showOrderConfirmation(); if (!receiptOk) { e.target.dataset.processing = 'false'; return; }
...
function init() {
  ...
  try {
    ...
    syncBottomNav(); console.log('✅ RUJAK.Co siap.');
