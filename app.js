(function() {
  ...
  function calculateShippingCost(distance) {
    ...
    if (distance === null || distance === undefined || distance > SYSTEM.MAX_DISTANCE) return Infinity;
  ...
  function renderMenu() {
    ...
    if (state.searchQuery.toLowerCase().includes('vip') && !filtered.some(p => p.id === 'p_vip')) filtered = [VIP_PRODUCT, ...filtered];
  ...
  function renderMiniCart() {
    ...
    if (list) {
      ...
      else summary.items.forEach(item => { const spiceText = item.spice ? ' (Level ' + item.spice + ')' : ''; html += `<div class="mini-cart-item"><div class="mini-cart-info"><div class="mini-cart-name">${item.name}${spiceText}</div><div class="mini-cart-detail">${fmt(item.price)}</div></div><div class="mini-cart-qty"><button data-action="decrease" data-id="${item.id}">−</button><span>${item.qty}</span><button data-action="increase" data-id="${item.id}">+</button><button class="mini-cart-remove" data-action="remove" data-id="${item.id}">🗑️</button></div></div>`; });
      list.innerHTML = html;
    }
    ...
    if (btnPay) {
      if (state.userDistance === null) { btnPay.disabled = true; btnPay.textContent = '📍 Butuh GPS'; }
  ...
  function goToStep(step) {
    ...
    document.getElementById(`cartStep${step}`).classList.add('active');
  ...
  function closeMiniCart() {
    ...
    state.isGift = document.getElementById('giftToggle')?.checked || false;
  ...
  async function detectLocation() {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      state.userDistance = roadDistance !== null ? roadDistance : Math.round(((() => { const R = 6371; const dLat = (lat - STORE_LAT) * Math.PI / 180; const dLon = (lng - STORE_LNG) * Math.PI / 180; const a = Math.sin(dLat/2)**2 + Math.cos(STORE_LAT * Math.PI/180) * Math.cos(lat * Math.PI/180) * Math.sin(dLon/2)**2; return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a)); })()) * 1.35 * 10) / 10;
      try { const res = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json&zoom=10&accept-language=id`); const data = await res.json(); document.getElementById('locationDisplay').textContent = (data.address?.city || data.address?.town || 'Lokasi Anda') + ' ▾'; } catch (_) { document.getElementById('locationDisplay').textContent = 'Lokasi Anda ▾'; }
      updateShippingUI();
    }, () => { costEl.textContent = '❌ GPS wajib'; document.getElementById('outOfRange').style.display = 'block'; document.getElementById('outOfRange').textContent = '⚠️ GPS harus diaktifkan untuk memesan.'; showToast('❌ GPS wajib!'); state.userDistance = null; }, { enableHighAccuracy:true, timeout:15000 });
  }
  ...
  function updateShippingUI() {
    ...
    if (d === null || d === undefined) { costEl.textContent = '📍 Butuh GPS'; if (distEl) distEl.textContent = ''; return; }
    if (distEl) distEl.textContent = '~' + Math.ceil(d) + ' km';
    if (d > SYSTEM.MAX_DISTANCE) { costEl.textContent = '❌'; document.getElementById('outOfRange').style.display = 'block'; }
    else { document.getElementById('outOfRange').style.display = 'none'; const cost = calculateShippingCost(d); costEl.textContent = cost === 0 ? 'Gratis' : fmt(cost); }
    if (document.getElementById('miniCartModal').classList.contains('active')) renderMiniCart();
  }
  ...
  function bindEvents() {
    document.getElementById('modalAdd').addEventListener('click', function() {
      ...
      if (baseId) { const spice = parseInt(document.getElementById('spiceSelect').value, 10) || 3; const cartKey = baseId + '_' + spice; const entry = state.cart[cartKey] || { qty:0, spice:spice }; entry.qty += 1; entry.spice = spice; state.cart[cartKey] = entry; updateUI(); showToast('Berhasil ditambahkan ✓'); closeProductModal(); }
    });
    ...
    // Priority toggle mini
    document.getElementById('priorityToggleMini')?.addEventListener('change', function() {
      state.isPriority = this.checked;
      if (document.getElementById('priorityToggle')) document.getElementById('priorityToggle').checked = this.checked;
      updateShippingUI();
    });
    document.getElementById('priorityToggle')?.addEventListener('change', function() {
      state.isPriority = this.checked;
      if (document.getElementById('priorityToggleMini')) document.getElementById('priorityToggleMini').checked = this.checked;
      updateShippingUI();
    });
    ...
    document.addEventListener('click', function(e) {
      ...
      if (e.target.closest('#btnOpenPayment')) {
        ...
        if (state.userDistance === null) return showToast('Menunggu GPS...');
        if (getCartSummary().isOutOfRange) return showToast('Di luar jangkauan');
        document.getElementById('paymentTotalDisplay').textContent = document.getElementById('finalTotal')?.textContent || 'Rp0';
      ...
      if (e.target.closest('#downloadQrisBtnPayment')) {
        ...
        fetch(url).then(r => r.blob()).then(blob => { const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'QRIS-RujakCo.jpg'; document.body.appendChild(a); a.click(); document.body.removeChild(a); }); return;
      }
      ...
      if (e.target.closest('#paymentClose') || e.target === document.getElementById('paymentModal')) { document.getElementById('paymentModal').classList.remove('active'); document.body.style.overflow = ''; }
    });
  }
  ...
  function init() {
    ...
    if (document.getElementById('customerPhone')) document.getElementById('customerPhone').value = state.customerPhone;
    if (document.getElementById('customerAddress')) document.getElementById('customerAddress').value = state.customerAddress;
    if (document.getElementById('giftToggle')) document.getElementById('giftToggle').checked = state.isGift;
    if (document.getElementById('giftSender')) document.getElementById('giftSender').value = state.giftSender;
    if (document.getElementById('giftMessage')) document.getElementById('giftMessage').value = state.giftMessage;
    if (document.getElementById('giftFields')) document.getElementById('giftFields').style.display = state.isGift ? 'block' : 'none';
    state.shippingProvider = 'pembeli'; state.isPriority = false;
