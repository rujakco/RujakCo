(function() {
  ...
  // ============================================================
  // RUJAK.CO — ULTIMATE SILENT LUXURY ENGINE (100% LENGKAP)
  // ============================================================
  ...
  const PRODUCTS = [
    { id:'p_m1', name:'Rujak Segar', desc:'Kombinasi buah pilihan dengan sambal original Rujak.Co. Disiapkan seketika.', price:35000, cat:'classic', container:'Thinwall 1000ml', size:'Reguler', sambal:'Original (1 Cup)', defaultSpice:3, image:'https://dk1tnyskaoive0dn.public.blob.vercel-storage.com/rujak-segar-hd.webp' },
    { id:'p_m2', name:'Rujak Serut', desc:'Buah diserut halus untuk pengalaman rasa yang lebih menyatu secara intim.', price:26000, cat:'classic', container:'Thinwall 750ml', size:'Reguler', sambal:'Original', defaultSpice:3, image:'https://dk1tnyskaoive0dn.public.blob.vercel-storage.com/rujak-serut-hd.webp' },
    { id:'p_m3', name:'Rujak Gaco', desc:'Enam buah pilihan dengan sambal mete premium. Mahakarya harmoni rasa.', price:40000, cat:'signature', container:'Thinwall 1000ml', size:'Eksklusif', sambal:'Mete (1 Cup)', defaultSpice:3, image:'https://dk1tnyskaoive0dn.public.blob.vercel-storage.com/rujak-gaco-hd.webp' },
    { id:'p_m4', name:'Rujak Rama', desc:'Porsi melimpah untuk dinikmati bersama kerabat terdekat.', price:48000, cat:'signature', container:'Thinwall Jumbo 1000ml', size:'Sharing', sambal:'Mete (2 Cup)', defaultSpice:4, image:'https://dk1tnyskaoive0dn.public.blob.vercel-storage.com/rujak-rama-hd.webp' },
    { id:'p_m5', name:'Rujak Mahkota', desc:'Koleksi premium dengan anggur Shine Muscat pilihan.', price:85000, cat:'reserve', container:'Luxury Box', size:'Premium', sambal:'Mete (2 Cup)', defaultSpice:3, image:'https://dk1tnyskaoive0dn.public.blob.vercel-storage.com/rujak-mahkota-hd.webp' },
    { id:'p_m6', name:'Tampah Nusantara', desc:'Representasi kemegahan rasa dalam wadah tampah bambu artisanal.', price:200000, cat:'reserve', container:'Tampah Bambu Ø40cm', size:'Perjamuan', sambal:'Original & Mete', defaultSpice:3, image:'https://dk1tnyskaoive0dn.public.blob.vercel-storage.com/tampah-nusantara-hd.webp' }
  ];
  ...
  function calculateShipping(distance, mainQty) {
    ...
    if (dist > 50) return { cost: null, label: 'Admin' };
  ...
  function renderMenu() {
    ...
    container.innerHTML = items.map(p => `
      <div class="boutique-item" data-id="${p.id}">
        <img src="${p.image}" class="btq-img" loading="lazy" alt="${p.name}">
  ...
  function renderCart() {
    ...
    if(floatBtn) floatBtn.style.display = document.getElementById('productPage').style.display === 'block' ? 'none' : 'flex';
  }
  ...
  function updateUI() { try{localStorage.setItem('rj_crt_v4', JSON.stringify(state.cart));}catch(e){} renderMenu(); renderCart(); if(document.getElementById('miniCartModal')?.classList.contains('active')) renderMiniCart(); }
  ...
  function initAIChat() {
    ...
    const processMsg = () => {
      ...
      messages.innerHTML += `<div style="text-align:right; margin-bottom:12px;"><span style="background:var(--green); color:white; padding:10px 14px; border-radius:16px; border-bottom-right-radius:4px; display:inline-block; max-width:85%;">${escapeHTML(txt)}</span></div>`;
      input.value = ''; messages.scrollTop = messages.scrollHeight;
      setTimeout(() => { messages.innerHTML += `<div style="margin-bottom:12px;"><span style="background:white; border:1px solid var(--gray-200); padding:10px 14px; border-radius:16px; border-bottom-left-radius:4px; display:inline-block; max-width:85%; box-shadow:0 2px 8px rgba(0,0,0,0.02);">Pesan Anda telah kami terima. Pramutamu kami akan memastikan pengalaman terbaik untuk Anda.</span></div>`; messages.scrollTop = messages.scrollHeight; }, 600);
    };
  ...
  function bindEvents() {
    ...
    document.getElementById('addToCartPage')?.addEventListener('click', function() {
      ...
      updateUI(); showToast('Ditambahkan ke tas belanja.');
      closeProductPage();
    });
    ...
    document.getElementById('btnAutoDetect')?.addEventListener('click', function() {
      if(navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(pos => {
          ...
          state.selectedDistrict = ''; if(di) di.value = 'Lokasi GPS Tersinkronisasi';
          updateShippingUI();
        }, () => { showToast('GPS ditolak.'); }, { timeout: 8000 });
      }
    });
    ...
    document.addEventListener('click', e => {
      ...
      if(e.target.id === 'btnOpenPayment') {
        ...
        if(!name || !phone || !address || (!state.selectedDistrict && !state.userDistance)) { showToast('Lengkapi data pengiriman.'); return; }
      ...
      // AI Chat
      if(e.target.id === 'aiChatToggle') { e.preventDefault(); document.getElementById('aiChatBox').style.display = 'flex'; }
      if(e.target.id === 'aiChatClose') { document.getElementById('aiChatBox').style.display = 'none'; }
    });
  }

  function init() {
    initAIChat();
    try { const s = localStorage.getItem('rj_crt_v4'); if(s) state.cart = JSON.parse(s); } catch(e){}
    updateUI(); bindEvents();
  }
