...
export function initShippingController(domConfig, appState, config) {
  DOM = domConfig;
  state = appState;
  APP_CONFIG = config;
}
...
export function updateShippingUI() {
  ...
  if (!section) return;
  const { subtotal, mainProductQty } = getCartSummary(state.cart); // Akses langsung dari state
...
export function initDrawerDistrictDropdown() {
  ...
  const handleSearch = debounce(async (query) => {
    ...
    searchLoading = true; updateSpinner();
    ...
    try { results = await queuedSearch(query, signal); } 
      ...
      if (err.name !== 'AbortError') dropdown.innerHTML = '<div style="padding:16px;text-align:center;color:var(--danger);">Koneksi terputus.</div>';
      return;
    } finally {
      searchLoading = false; updateSpinner();
      if (searchAbortController && searchAbortController.signal === signal) searchAbortController = null;
    }
    ...
    if (results.length === 0) {
      ...
      if (searchFailCount >= 2) {
        dropdown.innerHTML = `
          ...
          <div role="option" tabindex="0" data-manual="true" style="text-align:center;color:var(--gold-text);font-weight:600;cursor:pointer;">
            Isi manual & konfirmasi via WhatsApp
          </div>`;
      } else { dropdown.innerHTML = '<div style="padding:16px;text-align:center;color:var(--danger);">Lokasi tak ditemukan.</div>'; }
      return;
    }
    searchFailCount = 0;
    dropdown.innerHTML = results.map((place) => {
      ...
      return `
        <div role="option" tabindex="0" data-lat="${place.lat}" data-lon="${place.lon}" data-name="${escapeHTML(displayNameRaw)}">
          <strong>${mainLabel}</strong><br>
          <span style="font-size:0.75rem;color:var(--gray-500);">${escapeHTML(displayNameRaw)}</span>
        </div>`;
    }).join('');
  ...
  gpsBtn.addEventListener('click', async () => {
    ...
    gpsBtn.disabled = true; gpsBtn.setAttribute('aria-busy', 'true'); gpsLoading = true; updateSpinner();
    try {
      ...
      gpsLoading = false; updateSpinner();
      gpsBtn.disabled = false; gpsBtn.setAttribute('aria-busy', 'false');
    }
  });
}
