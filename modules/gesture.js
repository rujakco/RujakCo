...
/**
 * Menyuntikkan DOM dan fungsi dari app.js
 */
export function initGesturesConfig(domConfig, closeFn) {
  DOM = domConfig;
  closeProductPageFn = closeFn;
}
...
export function initDetailGestures() {
  ...
  track.addEventListener('touchstart', (e) => {
    ...
    // Jangan proses swipe jika di ujung kiri/kanan layar (mencegah bentrok dengan back native HP)
    ...
    // Hanya izinkan pull-down jika scroll produk sedang berada di paling atas
  ...
  track.addEventListener('touchmove', (e) => {
    ...
    if (!gestureDetermined && (absDx > 8 || absDy > 8)) {
      if (absDx > absDy) { isPulling = false; return; } // Batal jika swipe lebih dominan ke samping
  ...
  track.addEventListener('touchend', (e) => {
    if (!isPulling || !activeSlide || !gestureDetermined) {
      isPulling = false; activeSlide = null; return;
    }
    const dy = e.changedTouches[0].clientY - startY;
    activeSlide.style.transition = 'all 0.3s ease';
    // Jika ditarik cukup jauh ke bawah, tutup halaman
    if (dy > 120 && closeProductPageFn) {
