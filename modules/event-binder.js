...
export function bindEvents() {
  ...
  document.addEventListener('click', async (e) => {
    ...
    const qtyPlus = e.target.closest('.qty-plus'); const qtyMinus = e.target.closest('.qty-minus'); if (qtyPlus || qtyMinus) { const pid = (qtyPlus || qtyMinus).dataset.pid; if (!state.drafts[pid]) return; if (qtyPlus) state.drafts[pid].qty++; else if (state.drafts[pid].qty > 1) state.drafts[pid].qty--; updateDraftUI(pid); return; }
    const addBtn = e.target.closest('.add-to-cart-btn'); if (addBtn) { if (addBtn.dataset.processing === 'true') return; addBtn.dataset.processing = 'true'; if (window.navigator?.vibrate) window.navigator.vibrate(APP_CONFIG.HAPTIC.HEAVY); const pid = addBtn.dataset.pid, idx = addBtn.dataset.idx, draft = state.drafts[pid]; const cartKey = `${pid}_spice${draft.spice}`; if (!state.cart[cartKey]) state.cart[cartKey] = { id: pid, qty: 0, spice: draft.spice }; state.cart[cartKey].qty += draft.qty; state.drafts[pid].qty = 1; updateDraftUI(pid); updateCartUI(); showToast('Sajian berhasil ditambahkan.'); const cartNav = document.querySelector('.nav-cart-wrapper'); if (cartNav) { cartNav.classList.remove('bump'); void cartNav.offsetWidth; cartNav.classList.add('bump'); } addBtn.classList.add('success-flash'); addBtn.dataset.originalLabel = addBtn.dataset.originalLabel || addBtn.textContent; addBtn.textContent = '✓ Berhasil Ditambahkan'; setTimeout(() => addBtn.classList.remove('success-flash'), APP_CONFIG.TIMING.MODAL_TRANSITION); setTimeout(() => { addBtn.textContent = addBtn.dataset.originalLabel; const step1 = document.getElementById(`step1_${idx}_${pid}`), step2 = document.getElementById(`step2_${idx}_${pid}`); if (step1 && step2) { step1.style.opacity = '0'; step1.style.display = 'block'; step2.style.display = 'none'; requestAnimationFrame(() => { step1.style.opacity = '1'; }); } addBtn.dataset.processing = 'false'; }, APP_CONFIG.TIMING.SUCCESS_FLASH); return; }
    if (e.target.id === 'emptyCartBrowse') { closeModal(DOM.miniCartModal); if (openProductPageFn) openProductPageFn(0); return; }
    if (e.target.closest('[data-action="confirm-wa"]')) { const btn = e.target.closest('[data-action="confirm-wa"]'); if (btn.dataset.processing === 'true') return; btn.dataset.processing = 'true'; btn.textContent = 'Mengirim...'; try { if (sendReceiptToWhatsAppFn) await sendReceiptToWhatsAppFn(); } finally { closeModal(DOM.paymentModal); btn.textContent = 'Validasi Reservasi'; btn.dataset.processing = 'false'; } return; }
    const actionBtn = e.target.closest('[data-action]'); if (actionBtn && !actionBtn.classList.contains('add-to-cart-btn') && !actionBtn.classList.contains('step-1-btn')) { const id = actionBtn.dataset.id, type = actionBtn.dataset.action; if (!state.cart[id]) return; if (type === 'increase') state.cart[id].qty++; else if (type === 'decrease') { if (state.cart[id].qty === 1) { showConfirmModal('Keluarkan Sajian?', 'Keluarkan sajian ini dari reservasi Anda?', () => { delete state.cart[id]; updateCartUI(); showToast('Sajian dikeluarkan.'); }); return; } state.cart[id].qty--; } updateCartUI(); return; }
    const logBtn = e.target.closest('.log-btn'); if (logBtn) { document.querySelectorAll('.log-btn').forEach(b => b.classList.remove('active')); logBtn.classList.add('active'); state.shippingProvider = logBtn.dataset.provider; DOM.lalamoveOptions.style.display = state.shippingProvider === 'paxel' ? 'none' : 'block'; DOM.paxelOptions.style.display = state.shippingProvider === 'paxel' ? 'block' : 'none'; updateShippingUI(); return; }
    const vehBtn = e.target.closest('.veh-btn'); if (vehBtn) { document.querySelectorAll('.veh-btn').forEach(b => b.classList.remove('active')); vehBtn.classList.add('active'); state.tier = vehBtn.dataset.tier; updateShippingUI(); return; }
    if (e.target.id === 'btnOpenPayment') { if (e.target.dataset.processing === 'true') return; e.target.dataset.processing = 'true'; if (!Object.keys(state.cart).length) { showToast('Reservasi kosong.'); e.target.dataset.processing = 'false'; return; } const phone = DOM.customerPhoneInput?.value.trim() || ''; const address = DOM.customerAddressInput?.value.trim() || ''; if (!validatePhone(phone)) { showToast('Nomor HP tidak valid.'); e.target.dataset.processing = 'false'; return; } if (!validateAddress(address)) { showToast('Lengkapi alamat.'); e.target.dataset.processing = 'false'; return; } if (!state.selectedDistrict && !state.selectedDistrictFull) { showToast('Pilih alamat tujuan.'); e.target.dataset.processing = 'false'; return; } if (state.userDistance == null) { let recovered = false; const addressToSearch = state.selectedDistrictFull || (state.selectedDistrict && state.customerAddress ? `${state.selectedDistrict}, ${state.customerAddress}` : '') || DOM.districtInput?.value?.trim() || state.customerAddress; if (addressToSearch) { try { const results = await queuedSearch(addressToSearch); if (results.length > 0) { const place = results[0]; const result = await getDrivingDistance(SYSTEM.STORE_LAT, SYSTEM.STORE_LNG, parseFloat(place.lat), parseFloat(place.lon)); state.userDistance = result.distance; state.haversineUsed = result.isHaversine; state.selectedDistrictFull = place.display_name; state.selectedDistrict = extractShortLocation(place.display_name); DOM.districtInput && (DOM.districtInput.value = place.display_name); saveCustomer(phone, address, place.display_name, result.distance); updateShippingUI(); recovered = true; } } catch (err) { console.warn('Auto-recover gagal:', err); } } if (!recovered) { showToast('Silakan pilih alamat pengantaran dari hasil pencarian di atas.'); e.target.dataset.processing = 'false'; return; } } if (showOrderConfirmationFn) { const receiptOk = await showOrderConfirmationFn(); if (!receiptOk) { e.target.dataset.processing = 'false'; return; } } e.target.dataset.processing = 'false'; return; }
